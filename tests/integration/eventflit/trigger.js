...
describe("Eventflit", function() {
  ...
  describe("#trigger", function() {
    it("should send the event when no callback is given", function(done) {
      ...
      eventflit.trigger("test_channel", "my_event", { some: "data "});
      setTimeout(function() {
    ...
    it("should send the event to a single channel", function(done) {
      ...
      eventflit.trigger("one", "my_event", { some: "data "}, done);
    });
    ...
    it("should send the event to multiple channels", function(done) {
      ...
      eventflit.trigger(["one", "two", "three"], "my_event", { some: "data "}, done);
    });
    ...
    it("should serialize arrays into JSON", function(done) {
      ...
      eventflit.trigger("one", "my_event", [1,2,4], done);
    });
    ...
    it("should not serialize strings into JSON", function(done) {
      ...
      eventflit.trigger("test", "test_event", "test string", done);
    });
    ...
    it("should add socket_id to the request body", function(done) {
      ...
      eventflit.trigger("test_channel", "my_event", { some: "data "}, "123.567", done);
    });
    ...
    it("should call back with the result", function(done) {
      ...
      eventflit.trigger("test_channel", "my_event", { some: "data "}, function(error, request, response) {
    ...
    it("should call back with a RequestError if Eventflit responds with 4xx", function(done) {
      ...
      eventflit.trigger("test_channel", "my_event", { some: "data "}, function(error, request, response) {
        ...
        expect(error.url).to.match(
          /^http:\/\/service.eventflit.com\/apps\/1234\/events\?auth_key=f00d&auth_timestamp=[0-9]+&auth_version=1\.0&body_md5=cf87d666b4a829a54fc44b313584b2d7&auth_signature=[a-f0-9]+$/
        );
    ...
    it("should allow channel names with special characters: _ - = @ , . ;", function(done) {
      ...
      eventflit.trigger("test_-=@,.;channel", "my_event", { some: "data "}, function(error, request, response) {
        expect(error).to.be(null);
        expect(response.statusCode).to.equal(200);
        done();
      });
    ...
    it("should throw an error if channel name is longer than 200 characters", function() {
      ...
      expect(function() {
        eventflit.trigger(channel, "test");
      }).to.throwError(function(e) {
        expect(e).to.be.an(Error);
    ...
    it("should respect the timeout when specified", function(done) {
      ...
      eventflit.trigger("test_channel", "my_event", { some: "data "}, "123.567", function(error, request, response) {
        ...
        expect(error.url).to.match(
          /^http:\/\/service.eventflit.com\/apps\/1234\/events\?auth_key=f00d&auth_timestamp=[0-9]+&auth_version=1\.0&body_md5=0478e1ed73804ae1be97cfa6554cf039&auth_signature=[a-f0-9]+$/
        );
